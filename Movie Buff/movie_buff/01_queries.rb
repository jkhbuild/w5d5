...
def harrison_ford
  ...
  # as a lead actor.
  # Movie.joins(:castings, :actors).select(:id, :title, "castings.ord", "actors.name").where("actors.name = 'Harrison Ford' AND castings.ord > 1").distinct
    Movie.joins(:actors).select(:id, :title).where("actors.name ='Harrison Ford' AND castings.ord > 1")
end
...
def biggest_cast
  ...
  Movie.joins(:actors).select(:id, :title).group(:id).order("COUNT(actors.id) DESC").limit(3)
