def eighties_b_movies
  ...
  Movie.select(:id, :title, :score, :yr).where("yr BETWEEN 1980 AND 1989 AND score BETWEEN 3 AND 5")
...
def cast_list(title)
  ...
  Actor.joins(:movies).where('title in (?)', title).select(:id, :name).order("castings.ord ASC")
